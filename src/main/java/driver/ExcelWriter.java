...
public class ExcelWriter {
	 ...
	 */
	public ExcelWriter(String caseFilePath, String resultFilepath) {
		...
		if (Origintype.equals(".xlsx")) {
			try {
				xssfWorkbookRead = new XSSFWorkbook(new File(caseFilePath));
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
		...
		if (Origintype.equals(".xls")) {
			try {
				hssfWorkbookRead = new HSSFWorkbook(new FileInputStream(new File(caseFilePath)));
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
		...
		// 截取结果表后缀名
		String resultType = resultFilepath.substring(resultFilepath.lastIndexOf("."));
		// 确定结果表格式为excel格式
	...
	public void writePassCell(int rowNo, int column, String value) {
		Row row = null;
		try {
			// 获取指定行
			row = sheet.getRow(rowNo);
		...
		// 设置单元格样式
		CellStyle passStyle = null;
		// 新建字体样式
		Font font = null;
		// 根据不同的excel版本进行实例化
		if (hssfWorkbook != null) {
			font = hssfWorkbook.createFont();
			passStyle = hssfWorkbook.createCellStyle();
		} else {
			font = xssfWorkbook.createFont();
			passStyle = xssfWorkbook.createCellStyle();
		}
		// 设置字体颜色为红色
